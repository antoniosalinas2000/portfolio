...
export default function SkillCard(props: { image: string, title: string, level: number, color: string }) {
    ...
    return (
        <div className="flex flex-col items-center bg-white/20 border border-white rounded-md p-4 gap-y-4 w-60 h-72">
            <p className="text-4xl font-iceland text-white text-center">{props.title}</p>
            {/* Image container with fixed height */}
            <div className="h-36 w-full flex items-center justify-center">
                <Image src={props.image} width={150} height={150} alt="project" className="object-contain" />
            </div>
